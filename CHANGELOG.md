...
* [OLMIS-8350](https://openlmis.atlassian.net/browse/OLMIS-8350): Quantities in packs now print in the format the stock views use, `0 ( +79 )` instead of `0 (+79)`, and a negative remainder no longer prints a stray plus sign (`0 ( -5 )`). Applies to the stock card, stock card summary, stock event and physical inventory reports.
...
* [OLMIS-8224](https://openlmis.atlassian.net/browse/OLMIS-8224) Jasper Report translations improvements
  * Added the migration for `physicalInventory.jrxml` (translation keys update)
  * A failure of the report service is surfaced. The stock card, stock card summary, stock event and physical inventory print endpoints previously answered `200` with an empty body when `POST /api/reports/generate` failed, which reached the user as a blank PDF.
  * Translated labels wrap instead of being cut off. The Portuguese stock card rendered three different fields identically as `Código do`, and in Spanish and Portuguese the page number disappeared from the stock card and stock card summary footers, leaving `Página    de 3`.
  * The stock card summary page-number field was widened so `Página 1` fits on one line. At 40px it wrapped instead of being cut off, leaving `Página` on the footer line and the digit alone on a second line 3px above the bottom edge of the sheet, inside the unprintable margin.
  * The stock card page header no longer pairs a label with the wrong value. Its labels stretch and float while the values next to them do not, and Jasper only floats an element when something above it overlaps horizontally, so the label column slid down and the value column stayed put: in French, Spanish and Portuguese `Code de l'établissement:` came to rest beside the lot code and `Code du programme:` beside the expiry date. The header rows are now at a fixed position with a 30px pitch and the labels are wide enough for the translations, so a label and its value always share a line.
  * The stock card `Stock on Hand:` label no longer prints over its own value. The label box (x=490, width 40) overlapped the value box (x=520) by 10px, which was harmless in English and destructive in Spanish, where `Existencias` reaches 6.5px into the figure. The value also extended 5px past the right edge of the page.
  * The stock card summary title no longer gets overprinted when the injected global header wraps onto a second line.
