...
==================
* [SELV3-868](https://openlmis.atlassian.net/browse/SELV3-868): Fixed stock-based requisition figures after a cancellation. The reversing line item is now dated on the movement it cancels rather than the current date, and the cancel-tagged reasons also carry the tag of the movement they counter (`distributed`/`received`). The negative-stock check is consequently stricter: stock already issued can no longer be un-received.
