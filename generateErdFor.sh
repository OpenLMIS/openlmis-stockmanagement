...
docker pull openlmis/stockmanagement
...
&& mkdir output \
&& chmod 777 output \
&& (docker run --rm --network openlmisstockmanagementerdgeneration_default -v $WORKSPACE/output:/output schemaspy/schemaspy:snapshot -t pgsql -host db -port 5432 -db open_lmis -s stockmanagement -u postgres -p $DBPASSWORD -I "(data_loaded)|(schema_version)|(jv_.*)" -norows -hq &) \
&& sleep 30 \
...
&& zip -r $WORKSPACE/erd-stockmanagement.zip . \
&& popd \
&& rmdir output \
&& rm .env \
