...
import static org.openlmis.stockmanagement.i18n.MessageKeys.NOTIFICATION_HIGHSTOCK_CONTENT;
import static org.openlmis.stockmanagement.i18n.MessageKeys.NOTIFICATION_HIGHSTOCK_SUBJECT;
...
@Component
public class HighStockNotifier {
  ...
  /**
   * Notify users with a certain right for the facility/program that facility has low stock of a
   * product.
   *
   * @param stockCard StockCard for a product
   * @param rightId right UUID
   */
  public void notifyStockEditors(StockCard stockCard, UUID rightId, int averageConsumption) {
    NotificationMessageParams params = new NotificationMessageParams(
        getMessage(NOTIFICATION_HIGHSTOCK_SUBJECT),
        getMessage(NOTIFICATION_HIGHSTOCK_CONTENT),
        constructSubstitutionMap(stockCard, averageConsumption));
    stockCardNotifier.notifyStockEditors(stockCard, rightId, params);
  }

  Map<String, String> constructSubstitutionMap(StockCard stockCard, int averageConsumption) {
    ...
    valuesMap.put("averageConsumption",  String.valueOf(averageConsumption));
