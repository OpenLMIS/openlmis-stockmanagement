...
public class StockEventsAdapterBuilder {
  ...
  private List<StockEventLineItemDto> createAllStockEventLineItemDtos(StockEventExternalDto externalDto,
      Profiler profiler) {
    ...
    for (StockEventLineItemExternalDto item : externalDto.getItems()) {
      deferredLineItems.add(new DeferredLineItem(orderableLoader.deferredLoad(item.getOrderable()),
          lotLoader.deferredLoad(item.getLot()), item.getOccurredDate(), item.getQuantity(),
          reasonLoader.deferredLoad(item.getReason())));
    }
    ...
    return deferredLineItems.stream().map(loadedLineItem -> StockEventLineItemDto.builder()
        ...
        .lotId(ofNullable(loadedLineItem.getLot()).map(LotDto::getId).orElse(null))
